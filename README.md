...
```bash
ghq get https://github.com/kurotaky/suzuri-mcp-server.git
cd $(ghq root)/github.com/kurotaky/suzuri-mcp-server
npm install
npm run build
```
...
`/path/to/suzuri-mcp-server` は実際のパスに置き換えてください（例: `ghq root` の場合は `$(ghq root)/github.com/kurotaky/suzuri-mcp-server/dist/index.mjs`）。
